 ...
 * - conditional query building using init() method to build queries of whatever comlexity,
...
class DB
{
    ...
    /**
     * Function to init placeholders either in the full query or a query part
     ...
     *
     * Examples:
     * $query = $db->init("SELECT * FROM table WHERE foo=?s AND Bar=?s", $foo, $Bar);
     * echo $query;
     *
     * if ($foo) {
     *     $qpart = $db->init(" AND foo=?s", $foo);
     * }
    ...
    private function error($err)
    {
        ...
        if ( $this->emode == 'error' )
        {
            $err .= ". ServerError initiated in ".$this->caller().", thrown";
